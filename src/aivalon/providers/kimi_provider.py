"""Kimi (Moonshot AI) provider using OpenAI-compatible client."""
...
class KimiProvider(ModelProvider):
    ...
    def json_capability(self) -> JsonCapability:
        """Kimi supports JSON object mode but not strict JSON schema."""
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        if not self.api_key:
            raise ProviderError("KIMI_API_KEY or MOONSHOT_API_KEY is not set")
    ...
    def __enter__(self) -> "KimiProvider":
        return self
    ...
    def call_phase(self, request: ProviderRequest[T]) -> ProviderResponse[T]:
        """Call the Kimi API for a phase and validate the response."""
        ...
        for attempt in range(2):
            try:
                ...
                if attempt == 1:
                    ...
                    logger.warning("kimi.retrying_with_tool_reminder", attempt=attempt + 1)
                ...
                logger.error("kimi.api_error", error=str(exc), attempt=attempt + 1)
            ...
            try:
                ...
                logger.warning(
                    "kimi.schema_validation_failed",
                    errors=getattr(exc, "errors", exc.args),
                    attempt=attempt + 1,
                )
                if attempt == 1:  # Last attempt - use fallback
                    logger.error("kimi.falling_back_to_default", phase=request.phase)
                    payload_model = self._create_fallback_payload(request)
                    violations = ["fallback_used"]
                    break
                continue
            except (ValueError, KeyError) as exc:
                logger.warning("kimi.payload_parse_failed", error=str(exc), attempt=attempt + 1)
                if attempt == 1:  # Last attempt - use fallback
                    logger.error("kimi.falling_back_to_default", phase=request.phase)
                    payload_model = self._create_fallback_payload(request)
                    violations = ["fallback_used"]
        ...
        if violations:
            logger.info("kimi.payload_adjusted", violations=violations)
        ...
        return ProviderResponse(
            ...
            reasoning=None,  # Kimi doesn't provide reasoning tokens
    ...
    def _parse_payload(self, request: ProviderRequest[T], data: Dict[str, Any]) -> tuple[T, List[str]]:
        ...
        if not message:
            raise ValueError("Kimi response missing 'message'")

        tool_calls = message.get("tool_calls") or []
        if not tool_calls:
            raise ValueError("Kimi response missing 'tool_calls'")

        selected_call = self._select_tool_call(request, tool_calls)
        function = selected_call.get("function", {})
        arguments = function.get("arguments")
        if arguments is None:
            raise ValueError("Kimi tool call missing arguments")

        try:
            parsed_payload = orjson.loads(arguments)
        except orjson.JSONDecodeError as exc:
            raise ValueError(f"Kimi tool arguments were not valid JSON: {exc}") from exc
    ...
    def _maybe_filter_leaks(
        ...
        if sanitized != speech:
            logger.info("kimi.leak_sanitised", original=speech, sanitized=sanitized, seat=seat)
    ...
    def _select_tool_call(
        ...
        if tool_calls:
            return tool_calls[0]
        raise ValueError("Kimi response did not include any tool calls")
...
# Register Kimi provider with the factory
