"""DeepSeek provider using OpenAI-compatible client."""
...
# DeepSeek API base URL (with beta for strict mode)
DEFAULT_BASE_URL = "https://api.deepseek.com/v3.1_terminus_expires_on_20251015"
...
class DeepSeekProvider(ModelProvider):
    ...
    def json_capability(self) -> JsonCapability:
        """DeepSeek supports strict JSON schema in beta mode."""
    ...
    def call_phase(self, request: ProviderRequest[T]) -> ProviderResponse[T]:
        """Call the DeepSeek API for a phase and validate the response."""
        ...
        try:
            ...
            logger.error("deepseek.api_error", error=str(exc))
        ...
        try:
            ...
            logger.warning(
                "deepseek.schema_validation_failed",
                errors=getattr(exc, "errors", exc.args),
            )
            raise
        except (ValueError, KeyError) as exc:
            logger.warning("deepseek.payload_parse_failed", error=str(exc))
            raise ProviderError(f"Failed to parse DeepSeek payload: {exc}") from exc

        logger.info(
            "deepseek.call_success",
        ...
        # Log raw response to markdown file for debugging (same as OpenRouter and DashScope)
        self._log_raw_response_to_file(request, raw_response)

        if violations:
            logger.info("deepseek.payload_adjusted", violations=violations)
        ...
        return ProviderResponse(
            ...
            reasoning=None,  # DeepSeek doesn't provide reasoning tokens
    ...
    def _parse_payload(self, request: ProviderRequest[T], data: Dict[str, Any]) -> tuple[T, List[str]]:
        ...
        if not tool_calls:
            raise ValueError("DeepSeek response missing 'tool_calls'")

        selected_call = self._select_tool_call(request, tool_calls)
        function = selected_call.get("function", {})
        arguments = function.get("arguments")
        if arguments is None:
            raise ValueError("DeepSeek tool call missing arguments")

        try:
            parsed_payload = orjson.loads(arguments)
        except orjson.JSONDecodeError as exc:
            raise ValueError(f"DeepSeek tool arguments were not valid JSON: {exc}") from exc
    ...
    def _maybe_filter_leaks(
        ...
        if sanitized != speech:
            logger.info("deepseek.leak_sanitised", original=speech, sanitized=sanitized, seat=seat)
    ...
    def _select_tool_call(
        ...
        if tool_calls:
            return tool_calls[0]
        raise ValueError("DeepSeek response did not include any tool calls")
    ...
    def _log_raw_response_to_file(self, request: ProviderRequest[T], raw_response: Dict[str, Any]) -> None:
        ...
        leader_info = 'N/A'  # DeepSeek doesn't have leader info in metadata
...
# Register DeepSeek provider with the factory
